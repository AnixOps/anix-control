...
go test -v -race -coverprofile=coverage/go-coverage.out -covermode=atomic ./internal/... ./tests/...

# 生成 HTML 报告
echo -e "${BLUE}生成 Go 覆盖率 HTML 报告...${NC}"
go tool cover -html=coverage/go-coverage.out -o coverage/go-coverage.html

# 显示覆盖率摘要
echo -e "${GREEN}Go 测试覆盖率摘要:${NC}"
...
echo "前端覆盖率报告: coverage/frontend-coverage/"
echo "==========================================${NC}"