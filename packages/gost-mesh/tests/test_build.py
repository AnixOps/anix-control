...
class GostMeshPackageTest(unittest.TestCase):
    ...
    def test_manifest_and_archive_bind_agent_config_and_webui(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_root:
            ...
            self.assertEqual(
                ["forward.gost.mesh", "forward.tunnel.wss", "forward.tunnel.tuic", "forward.tunnel.quic"],
                manifest["capabilities"],
            )
            self.assertEqual(
                ["health_interval_seconds", "listen_address", "listen_port", "protocol_profile", "tls_cert_secret", "tls_key_secret", "upstream_policy"],
                manifest["config_schema"]["required"],
            )
            self.assertEqual("/api/v3/plugins/gost-mesh/status", manifest["control_routes"][0])
