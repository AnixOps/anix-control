#!/usr/bin/env bash
# Execute the official nftables Forward package release contract without
# persisting or uploading a signing private key.

set -Eeuo pipefail

readonly ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
readonly PACKAGE_ROOT="${ROOT_DIR}/packages/nftables-forward"
...
main() {
  ...
  [[ "${agent_version}" == "nftables-forward 1.2.0" ]] || fail "Agent binary is not nftables-forward 1.2.0: ${agent_version}"
  ...
  for name in nftables-forward-1.2.0.tar manifest.json build-report.json SHA256SUMS.txt; do
  ...
  "${SIGNING_SCRIPT}" \
    ...
    --artifact "${first}/nftables-forward-1.2.0.tar" \
  ...
  expect_failure "tampered signature" \
    ...
    --public-key "${public_key}" \
    --openssl "${OPENSSL_BIN}"
  cp "${first}/nftables-forward-1.2.0.tar" "${work}/artifact-tampered.tar"
