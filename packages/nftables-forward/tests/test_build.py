...
SPEC = importlib.util.spec_from_file_location("nftables_forward_build", PACKAGE_ROOT / "build.py")
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("cannot load nftables-forward build module")
BUILD = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = BUILD
SPEC.loader.exec_module(BUILD)


class NftablesForwardPackageTest(unittest.TestCase):
    def make_agent(self, root: Path) -> BUILD.AgentInput:
        path = root / "nftables-forward-agent"
        path.write_bytes(b"#!/bin/sh\nprintf 'nftables-forward-test\\n'\n")
        path.chmod(0o755)
    ...
    def test_webui_is_dependency_free(self) -> None:
        ...
        self.assertIn("const STATUS_PATH = '/api/v3/plugins/nftables-forward/status?limit=200'", text)
        self.assertIn("Dedicated TCP/UDP package", text)
