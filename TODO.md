...
Current scope: panel peer custody, native WireGuard subscription output, sing-box WireGuard outbound output, runtime peer fields for V2bX, and unit coverage are implemented. Full dual-node GOST relay+QUIC/WSS orchestration, admin UI, online/limit behavior, migration dry-run evidence, and GitHub Actions relay-path verification are still pending.
