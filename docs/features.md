...
| WireGuard | WSS compatibility tunnel mode | Partial | `tunnel_type=wss`/`relay.wss_compat` contract and V2bX command selection | Add admin one-click switch and real compatibility evidence; WSS is compatibility mode, not the default. |
...
| P0 WireGuard dual-node relay | Partial/P0 | Panel peer custody, native/sing-box subscription output, UniProxy/gRPC peer runtime fields, relay TUN contract fields, V2bX entry interface, traffic delta parsing, initial peer online-state reporting, GOST TUN command planning, entry policy routing, and exit NAT command application exist, but there is still no admin UI, no real dual-node GOST relay+QUIC/WSS tunnel evidence, no speed-limit integration evidence, and no GitHub Actions relay-path verification. |
