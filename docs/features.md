...
| WireGuard | WireGuard user access over dual-node relay | Partial/P0 | `ProtocolWireGuard`, `v2_wireguard_peer`, native `.conf` formatter, sing-box outbound, UniProxy/gRPC runtime peer fields, relay TUN contract fields, admin WireGuard visual protocol form, V2bX GOST TUN runtime tests, `docs/guide/wireguard-relay.md` | Panel peer custody, subscription output, node runtime peer contract, first admin visual configuration flow, V2bX entry interface, traffic deltas, initial peer online-state reporting, GOST TUN command planning, entry policy routing, and exit NAT command application have tested slices. Real dual-node relay evidence, speed-limit behavior, API validation hardening, integration tests, and GitHub Actions relay verification are still required before marking production-complete. |
| WireGuard | WireGuard subscription output for Shadowrocket/Loon/v2rayN | Partial | Native WireGuard `.conf` output and sing-box WireGuard outbound are implemented and unit-tested | Verify Shadowrocket, Loon, and v2rayN import behavior against real client formats before marking fully implemented. |
| WireGuard | GOST relay+QUIC tunnel backend | Partial/P0 | Panel node config carries `tunnel_type=quic`, relay TUN defaults, and V2bX has entry/exit command tests | This is the default entry-to-exit transport; real two-machine relay-path evidence is still required. |
| WireGuard | WSS compatibility tunnel mode | Partial | `tunnel_type=wss`/`relay.wss_compat` contract, admin WireGuard visual WSS switch, and V2bX command selection | Add real compatibility evidence; WSS is compatibility mode, not the default. |
...
| Operations | Release workflow with artifacts, checksums, SBOM | Partial | GitHub Actions release workflow is the required release build source; CI now guards strict tag gating, release prerequisites, multi-platform binaries, Docker metadata, migration dry-run evidence, frontend archives, self-tested release manifest, deterministic `RELEASE_NOTES.md`, checksum/manifest consistency verification, SBOM, operator deployment and upgrade runbooks, and GitHub-generated release notes | Run and verify an actual tag release before marking complete. |
...
| P0 WireGuard dual-node relay | Partial/P0 | Panel peer custody, native/sing-box subscription output, UniProxy/gRPC peer runtime fields, relay TUN contract fields, first admin visual protocol form, V2bX entry interface, traffic delta parsing, initial peer online-state reporting, GOST TUN command planning, entry policy routing, and exit NAT command application exist, but there is still no hardened API validation workflow, no real dual-node GOST relay+QUIC/WSS tunnel evidence, no speed-limit integration evidence, and no GitHub Actions relay-path verification. |
