...
| 3.3 tunnel mesh and NAT egress | Preview/Partial | The `nat-egress` package contract, real Agent runtime, policy-routing/NAT namespace acceptance, crash-safe cleanup, immutable Agent pin, and production signing workflow wiring exist. The real `gost-mesh` WSS/TUIC/QUIC runtime remains absent, and both plugins still need canary evidence. |
