...
| Platform | `nftables-forward` official plugin runtime | Preview/Partial | Signed 1.1.0 package with one Control/Agent schema, safe `apply=false` defaults, semantic topology preflight, `plugin.runtime-state`/`plugin.cleanup`, durable pre-apply table snapshots, signed cleanup/validate modes, TCP/UDP namespace traffic, exact rollback, and `SIGKILL` plus Agent-restart recovery | Add live kernel ruleset hash/counter observations, staged restore/fallback records, IPv4/IPv6 rollout evidence, and a sustained canary before production cutover. |
