...
| Operations | Release workflow with artifacts, checksums, SBOM | Partial | GitHub Actions release workflow is the required release build source; CI now guards strict tag gating, release prerequisites, multi-platform binaries, Docker metadata, migration dry-run evidence, frontend archives, checksums, SBOM, operator runbook, and release notes | Run and verify an actual tag release before marking complete. |
