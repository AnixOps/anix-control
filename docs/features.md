...
| Operations | Deployment, upgrade, and production docs | Implemented | GitHub Actions release artifacts, guarded local `config/deploy/deploy_panel.sh`, guarded legacy `config/scripts/deploy.sh` and `config/scripts/pre-deploy.sh`, guarded V2bX Ansible rollout helper, CI release-build policy check, safe local artifact cleanup helper, `docs/DEPLOYMENT.md`, `docs/UPGRADE.md` | Keep local build guard, cleanup helper, artifact deployment docs, and upgrade rollback docs aligned. |
