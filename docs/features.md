...
| Platform | Plugin microkernel and signed package catalog | Preview/Partial | `/api/v3`, `v3_kernel_*`, official manifest verification, immutable artifact repository, installation intent, canonical operation envelopes, opt-in durable Control/Agent lifecycle dispatch, lease/cancel/restart replay, monotonic topology observed-state write-back, recursive dependency preflight, cross-repository process E2E, PostgreSQL restore rehearsal, package release contract, and fail-closed backend plugin route admission | Add dependency-aware graph execution/rollback, real topology executor fan-out, staging restore smoke, and production secret-backed package signing/upload before 3.1 promotion. |
| Platform | Package-driven pluggable WebUI | Preview/Partial | Signed WebUI metadata, verified extension catalog, immutable same-origin bundle storage, browser SHA-256 verification, dynamic namespaced menu/route registration, frontend/backend route permission enforcement, happy-dom lifecycle tests, local-module identity checks, installation configuration API, and Chromium route-isolation/fail-closed E2E | Run the same browser flow against a live Control catalog during staging; keep package runtime feature-gated until production signing/upload evidence exists. |
