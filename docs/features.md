...
| WireGuard | WireGuard user access over dual-node relay | Partial/P0 | `ProtocolWireGuard`, `v2_wireguard_peer`, native `.conf` formatter, sing-box outbound, UniProxy/gRPC runtime peer fields, V2bX v2.3.3 initial online-state reporting, `docs/guide/wireguard-relay.md` | Panel peer custody, subscription output, node runtime peer contract, V2bX entry interface, traffic deltas, and initial peer online-state reporting have tested slices. Full V2bX dual-node relay orchestration, GOST relay+QUIC, overseas exit NAT, speed-limit behavior, admin UI, integration tests, and GitHub Actions relay verification are still required before marking production-complete. |
...
| WireGuard | GOST relay+QUIC tunnel backend | Planned/P0 | Panel node config now carries `tunnel_type=quic` and relay defaults | Add V2bX entry/exit runtime support; this is the default entry-to-exit transport for the P0 WireGuard path. |
...
| P0 WireGuard dual-node relay | Partial/P0 | Panel peer custody, native/sing-box subscription output, UniProxy/gRPC peer runtime fields, V2bX entry interface, traffic delta parsing, and initial peer online-state reporting exist, but there is still no admin UI, no full dual-node GOST relay+QUIC/WSS tunnel orchestration evidence, no overseas exit NAT evidence, no speed-limit integration evidence, and no GitHub Actions relay-path verification. |
