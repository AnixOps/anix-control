...
| WireGuard | WireGuard user access over dual-node relay | Partial/P0 | `ProtocolWireGuard`, `v2_wireguard_peer`, native `.conf` formatter, sing-box outbound, `docs/guide/wireguard-relay.md` | Panel peer custody and subscription output have a first tested slice. V2bX domestic entry termination, GOST relay+QUIC, overseas exit NAT, traffic/online/limit behavior, admin UI, integration tests, and GitHub Actions relay verification are still required before marking production-complete. |
...
| P0 WireGuard dual-node relay | Partial/P0 | Panel peer custody and native/sing-box subscription output exist, but there is still no admin UI, no V2bX runtime application for domestic entry termination or overseas exit NAT, no live GOST relay+QUIC/WSS tunnel orchestration, no traffic/online/limit integration evidence, and no GitHub Actions relay-path verification. |
