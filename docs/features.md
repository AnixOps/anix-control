...
| WireGuard | WSS compatibility tunnel mode | Partial | `tunnel_type=wss`/`relay.wss_compat` contract, entry SNI/CA and exit certificate/key validation, admin WSS certificate form, V2bX verified command selection, and RC/tag namespace acceptance | Add successful Actions artifacts, production certificate rotation evidence, and real cross-region compatibility evidence; WSS is compatibility mode, not the default. |
