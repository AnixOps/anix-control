...
| WireGuard | WireGuard user access over dual-node relay | Partial/P0 | `ProtocolWireGuard`, `v2_wireguard_peer`, native `.conf` formatter, sing-box outbound, UniProxy/gRPC runtime peer fields, `docs/guide/wireguard-relay.md` | Panel peer custody, subscription output, and node runtime peer contract have a tested slice. Full V2bX dual-node relay orchestration, GOST relay+QUIC, overseas exit NAT, online/limit behavior, admin UI, integration tests, and GitHub Actions relay verification are still required before marking production-complete. |
...
| P0 WireGuard dual-node relay | Partial/P0 | Panel peer custody, native/sing-box subscription output, and UniProxy/gRPC peer runtime fields exist, but there is still no admin UI, no full dual-node GOST relay+QUIC/WSS tunnel orchestration evidence, no overseas exit NAT evidence, no online/limit integration evidence, and no GitHub Actions relay-path verification. |
