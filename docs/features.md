...
| Operations | Deployment script and production deploy docs | Implemented | GitHub Actions release artifacts, guarded local `config/deploy/deploy_panel.sh`, safe local artifact cleanup helper, `docs/DEPLOYMENT.md` | Keep local build guard, cleanup helper, and artifact deployment docs aligned. |
