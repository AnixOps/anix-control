...
- Admin subscription management handler tests now assert panel `code/msg/ts/data` envelopes for groups, templates, protocols, formats, and preview responses; Subscriptions frontend tests cover legacy and enveloped group/template/protocol/preview payloads, with admin API mapping coverage for subscription management calls.
...
- Admin Agent handler tests now assert panel `code/msg/ts/data` envelopes for list, task-result, task-history, and monitor read responses; Agent frontend tests cover legacy, enveloped, and nested list/history/execute payloads, with admin API mapping coverage for Agent read and command routes.
