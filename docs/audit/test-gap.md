...
- P0 WireGuard dual-node relay has panel unit coverage for protocol config defaults, relay TUN contract defaults, native `.conf` output, sing-box WireGuard outbound output, peer allocation, UniProxy/gRPC runtime peer fields, and the first admin visual protocol form that generates QUIC/WSS relay settings. V2bX now has focused unit coverage for WireGuard interface config rendering, command planning, peer validation, `wg show <iface> transfer` traffic delta parsing, recent-handshake online-state parsing from `wg show <iface> dump`, GOST TUN relay entry/exit command planning, entry policy routing, and exit NAT command application. Gaps remain for hardened admin API validation, production migration dry-run evidence, Shadowrocket/Loon/v2rayN real-client compatibility, real domestic-entry and overseas-exit runtime tests, GOST relay+QUIC and WSS compatibility integration tests, speed-limit tests, and GitHub Actions verification for `WireGuard access -> domestic entry termination -> GOST relay+QUIC -> overseas exit NAT`.
