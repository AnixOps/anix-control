...
- User login handler tests now cover enabled-MFA challenge, successful TOTP challenge completion, invalid MFA attempts, login-attempt recording, global `enforce_for_all` and `enforce_for_admin` no-token enrollment-required responses, and regular-user access when only admins are covered; auth rate-limit and E2E fixtures migrate MFA/SystemConfig tables so the shared login path remains covered; Login frontend tests cover the two-step MFA challenge, missing-code validation, and MFA enrollment-required prompts that do not store a token.
...
- P0 WireGuard dual-node relay has panel unit coverage for protocol config defaults, relay TUN contract defaults, native `.conf` output, sing-box 1.13 endpoint output, link-only-client native fallback, peer allocation, exit-role peer exclusion, keypair generation, API validation, raw/custom runtime-config validation, IPv4-only relay guards, secure WSS entry/exit certificate contracts, UniProxy/gRPC runtime peer fields, runtime-health persistence, and the admin visual protocol form that generates QUIC/WSS relay settings. V2bX has focused unit coverage for WireGuard interface config rendering, command planning, peer validation, peer credential refresh, runtime-health defaults, `wg show <iface> transfer` traffic delta parsing, recent-handshake online-state parsing from `wg show <iface> dump`, HTTP parsing of exit configs without entry keys, relay-only exit behavior, secure WSS URL encoding, GOST TUN relay entry/exit command planning, entry policy routing, exit NAT command application, `tc` shaping, and dynamic-limit convergence. RC/tag publication now invokes a privileged GitHub Actions namespace matrix that exercises a real client WireGuard interface through V2bX entry routing, GOST QUIC or CA/SNI-verified WSS, exit NAT, and an HTTP target; no successful run artifact exists until the first workflow execution. Remaining evidence gaps are production migration dry-run evidence, Shadowrocket/Loon/v2rayN real-client compatibility, production certificate rotation, and geographically separated domestic-entry/overseas-exit runtime tests.
