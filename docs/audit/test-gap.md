...
- P0 WireGuard dual-node relay has panel unit coverage for protocol config defaults, native `.conf` output, sing-box WireGuard outbound output, peer allocation, and UniProxy/gRPC runtime peer fields. V2bX now has focused unit coverage for WireGuard interface config rendering, command planning, peer validation, `wg show <iface> transfer` traffic delta parsing, and recent-handshake online-state parsing from `wg show <iface> dump`. Gaps remain for admin API/UI validation, production migration dry-run evidence, Shadowrocket/Loon/v2rayN real-client compatibility, real domestic-entry and overseas-exit runtime tests, GOST relay+QUIC and WSS compatibility integration tests, speed-limit tests, and GitHub Actions verification for `WireGuard access -> domestic entry termination -> GOST relay+QUIC -> overseas exit NAT`.
