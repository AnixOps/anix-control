...
- User login handler tests now cover enabled-MFA challenge, successful TOTP challenge completion, invalid MFA attempts, login-attempt recording, and no-token challenge responses; auth rate-limit and E2E fixtures migrate MFA/SystemConfig tables so the shared login path remains covered; Login frontend tests cover the two-step MFA challenge and missing-code validation.
