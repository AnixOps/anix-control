...
- API error response structure is not fully uniform across all handlers.
  ...
  `status=ok` compatibility path, admin subscription group and template CRUD plus protocol
  binding/read success-user-error responses have handler, service not-found/missing-protocol,
  and Subscriptions frontend compatibility coverage, while format and preview success payloads remain covered and public
