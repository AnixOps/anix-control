...
- API error response structure is not fully uniform across all handlers.
  ...
  `status=ok` compatibility path, admin subscription group and template CRUD, protocol
  binding/read, and preview success-user-error responses have handler, service
  not-found/missing-protocol, and Subscriptions frontend compatibility coverage; admin subscription user/plan group binding
  success-user-error responses have handler and service missing-reference/missing-relation coverage, while format success payloads remain covered and public
  ...
  dashboard and user/order stats database-error paths now propagate service failures into panel errors, admin
  ...
  coverage plus service missing-reference coverage and localized Orders frontend/API compatibility coverage, admin
...
- Release assets include deterministic `RELEASE_NOTES.md` generated from `CHANGELOG.md`.
...
- Release notes generation is provided by `config/scripts/generate_release_notes.py` and covered by a CI self-test.
- Release artifact consistency is verified by `config/scripts/verify_release_artifacts.py` before GitHub Release creation.
- Release assets include `anix-control-source.sbom.spdx.json` generated by `anchore/sbom-action`.
- Release binaries and frontend assets must be produced by GitHub Actions; local source-tree deploy scripts and legacy deploy/pre-deploy entrypoints are guarded and are not approved release build paths.
- CI enforces the release-build policy guard for deployment shell scripts with `config/deploy/check_release_build_policy.sh`.
