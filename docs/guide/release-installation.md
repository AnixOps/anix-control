...
3. Decide the exact version to install. Pinning a tag makes the operation
   reproducible; the signed-package WebUI preview is `v4.0.0-alpha.6`.
...
In `v4.0.0-alpha.6`, `machine-telemetry` 1.1.0 still exercises the complete
...
`v4.0.0-alpha.6` is a canary release. Keep production traffic on the tested
