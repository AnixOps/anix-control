...
3. Decide the exact version to install. Pinning a tag makes the operation
   reproducible; the signed-package WebUI preview is `v4.0.0-alpha.5`.
...
In `v4.0.0-alpha.5`, `machine-telemetry` 1.1.0 still exercises the complete
metrics package path. The signed `nftables-forward` 1.1.0 package adds one
Control/Agent configuration contract, topology preview/diagnosis, and a
crash-safe ownership journal with signed cleanup. Verify install, observation
...
`v4.0.0-alpha.5` is a canary release. Keep production traffic on the tested
