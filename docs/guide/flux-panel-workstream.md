...
- Admin-side runtime controls remain outside the Flux-cloned `/admin/forward` page. `web/src/views/admin/System.vue` currently hosts `forward.runtime_backend`, `forward.runtime.ansible.backend`, `forward.runtime.ansible.config`, and `GET /api/v2/admin/forward/runtime/jobs` as the compatibility surface for the proprietary execution layer. In current code, `NodeX/gost` (stateful control-plane/agent path) and local ansible (stateless panel-host executor) are distinct runtime paths and must stay documented as such. Recommended local backend is `nftables_ansible`; `iptables_ansible` is legacy compatibility. The release installer preserves an existing runtime configuration and uses the version-matched template only for a fresh install.
- Current `gost` runtime calls for `panel_forward` and `legacy_rule` require an explicit NodeX control-plane base URL (`forward_runtime.nodex.base_url`) and no longer fall back to the ingress node transport for the outer control-plane hop; keep this requirement documented outside the Flux-cloned page.
