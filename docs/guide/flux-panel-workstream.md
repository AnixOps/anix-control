...
- Dual-runtime admin controls now live in `web/src/views/admin/System.vue` with `forward.runtime_backend`, `forward.runtime.iptables_ansible.config`, and `GET /api/v2/admin/forward/runtime/jobs`; an in-process worker started by `cmd/server/main.go` consumes queued `iptables_ansible` jobs. `install.sh` (`panel_install.sh` wrapper) and the Docker env bootstrap keys (`FORWARD_RUNTIME_BACKEND`, `FORWARD_RUNTIME_ANSIBLE_CONFIG_JSON`) are part of the same local extension surface and should not change the Flux-shaped `/admin/forward` page.
