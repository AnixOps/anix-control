...
| 运维工作台 | `/maintenance`，角色/设置/工单/操作 | 组件/导航回归、前端构建通过；最终集成 CI 待记录 | 待实际维护人员演练 |
...
| Control 独立外部监测 | 完成候选产品公开能力核对 | Free 双渠道不满足；没有配置或发送 | 未完成，禁止标记宕机告警交付 |

CI 最终 SHA/run URL 在本记录末尾更新。只有通过的实际 Actions 结果可标记 CI 验收；本地验证不能替代 CI，更不能替代生产可用性。
...
详细运行手册见 [MAINTENANCE_P0.md](MAINTENANCE_P0.md)。事件自由文本不保存，外部通知采用至少一次交付；发送完成但写回前进程崩溃可能重复通知。通知停用不停止故障事件持久化。数据表初始化会修改 schema，上线需要独立备份和迁移演练。
