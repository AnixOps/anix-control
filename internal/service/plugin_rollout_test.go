...
func TestRecordPackageValidationRequiresPersistedHostDigest(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = RecordMigrationCheckpoint(db, started.ID, PackageMigrationCheckpoint{
		OpaqueCheckpoint: "opaque-complete", ValidationDigest: "host-validation", Complete: true,
	})
	require.NoError(t, err)
