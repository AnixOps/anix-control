...
# 1. 运行部署前测�?
echo -e "${BLUE}[1/6] 运行部署前测�?..${NC}"
if [ -f "./config/scripts/pre-deploy.sh" ]; then
    if ./config/scripts/pre-deploy.sh; then
...
if [ -f "${DEPLOY_DIR}/${BINARY_NAME}" ]; then
    ...
    echo -e "${YELLOW}�?未找到旧版本，跳过备�?{NC}"
fi

# 5. 部署新版�?
echo -e "${BLUE}[5/6] 部署新版�?..${NC}"
...
if command -v systemctl &> /dev/null; then
    if systemctl is-active --quiet v2board; then
        ...
        echo -e "${GREEN}�?服务已重�?{NC}"
    else
        echo -e "${YELLOW}�?v2board 服务未安装，请手动配�?systemd${NC}"
    fi
else
    echo -e "${YELLOW}�?systemctl 不可用，请手动重启服�?{NC}"
fi

# 健康检�?
echo ""
echo -e "${BLUE}执行健康检�?..${NC}"
sleep 2
if curl -sf http://localhost:8080/health > /dev/null; then
    echo -e "${GREEN}�?应用运行正常${NC}"
else
    echo -e "${YELLOW}�?健康检查失败，请检查应用状�?{NC}"
fi

echo ""
echo "=========================================="
echo -e "${GREEN}�?部署成功!${NC}"
echo "二进制文�? ${DEPLOY_DIR}/${BINARY_NAME}"
echo "数据目录: ${DATA_DIR}"
